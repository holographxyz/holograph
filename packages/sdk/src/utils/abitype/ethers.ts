...
import type {IsUnknown, UnionToIntersection} from './types'
import type {AbiItemName, CountOccurrences, Event, GetOverridesForAbiStateMutability} from './'
...
export function getContract<const abi extends Abi | readonly unknown[]>({
  ...
  return new EthersContract(address, abi as unknown as InterfaceAbi, signerOrProvider) as GetContractResult<abi>
}

export type GetContractResult<TAbi = unknown> = TAbi extends Abi ? Contract<TAbi> & EthersContract : EthersContract
...
type BaseContract<
  TContract extends Record<keyof Pick<EthersContract, PropertyKeys | FunctionKeys | EventKeys>, unknown>,
...
type BaseInterface<Interface extends Record<keyof Pick<ContractInterface, InterfaceKeys>, unknown>> = Omit<
  ContractInterface,
  InterfaceKeys
> &
  Interface
  ...
  BaseContract<{
    ...
    emit<TEventName extends ExtractAbiEventNames<TAbi> | EventFilter>(
      ...
      ...args: AbiParametersToPrimitiveTypes<
        ExtractAbiEvent<TAbi, TEventName extends string ? TEventName : ExtractAbiEventNames<TAbi>>['inputs']
    ...
    listeners<TEventName extends ExtractAbiEventNames<TAbi>>(eventName: TEventName): Listener<TAbi, TEventName>[]
    listeners(
...
> = UnionToIntersection<
  {
    ...
    [K in keyof TAbi]: TAbi[K] extends infer TAbiFunction extends AbiFunction & {
      ...
      ? {
          ...
          [_ in CountOccurrences<TAbi, {name: TAbiFunction['name']}> extends 1
...
> = ({
  ...
  [_ in 'pure' | 'view']: TAbiFunction['outputs']['length'] extends infer TLength
    ? TLength extends 0
...
> = AbiParametersToPrimitiveTypes<TAbiEvent['inputs']> extends infer TArgs extends readonly unknown[]
...
type Filters<TAbi extends Abi> = UnionToIntersection<
  {
    [K in keyof TAbi]: TAbi[K] extends infer TAbiEvent extends AbiEvent
      ? {
          [_ in CountOccurrences<TAbi, {name: TAbiEvent['name']}> extends 1
