...
export function create2AddressFromDeploymentHash(configHash: Hex, factoryAddress: Address) {
  return ('0x' +
    keccak256(
      `0xff${remove0x(factoryAddress)}${remove0x(configHash)}${keccak256(remove0x(bytecodes.Holographer) as Hex)}`,
    )) as Address
}
