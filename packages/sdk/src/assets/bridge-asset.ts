...
export class BridgeAsset {
  ...
  protected async _estimateBridgeOutDestinationGas(
    ...
    if (gasController?.gasLimit) {
      ...
      const holographToChainId = getNetworkByChainId(toChainId).holographId
      const gasEstimationHelperAddress: Address = getGasEstimationAddress(toChainId)
      const maxGasValue: bigint = hexToBigInt(('0x' + 'ff'.repeat(32)) as Hex)
      const gasLimitStub: bigint = getTestGasLimit(toChainId) // this is a very big gas value so we can use to estimate with
